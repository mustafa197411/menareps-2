...
import { resolveSecuredAnalyticsScope, filterRecordByScopeAndFilters, paginateResults } from "./analyticsScopeEngine";
 ...
 */
export const syncAuthUserToProfile = functions.auth.user().onCreate(async (user: any) => {
 ...
 */
export const validateCommercialOrder = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "The function must be called by an authenticated operator."
    );
  }
 ...
 */
export const logCollectionAudit = functions.firestore
 ...
 * =========================================================================
 */
async function getCollectionArray(collectionName: string): Promise<any[]> {
...
async function enforceAuthAndResolveScope(context: functions.https.CallableContext) {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "Action denied. The user must be authenticated."
    );
  }
  try {
    return await resolveSecuredAnalyticsScope(context.auth.uid, db);
  } catch (error: any) {
 ...
 */
export const getAnalyticsSummary = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  const { scope } = await enforceAuthAndResolveScope(context);
  const filters: AnalyticsFilters = data.filters || {};
  ...
  // Filter records
  const filteredPhysicianVisits = physicianVisits.filter(v => filterRecordByScopeAndFilters(v, scope, filters));
  const filteredPharmacyVisits = pharmacyVisits.filter(v => filterRecordByScopeAndFilters(v, scope, filters));
  const filteredOrders = orders.filter(o => filterRecordByScopeAndFilters(o, scope, filters));
 ...
 */
export const getProductPerformanceAnalytics = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  const { scope } = await enforceAuthAndResolveScope(context);
  const filters: AnalyticsFilters = data.filters || {};
  ...
  const performanceItems = products.map(product => {
    ...
    const detailingVisits = physicianVisits.filter(v => {
      ...
      return isDetailed && filterRecordByScopeAndFilters(v, scope, filters);
    });
    ...
    orders.forEach(order => {
      if (filterRecordByScopeAndFilters(order, scope, filters) && order.items) {
  ...
  const paginated = paginateResults(performanceItems, page, pageSize);
 ...
 */
export const getUnvisitedCustomersAnalytics = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  const { scope } = await enforceAuthAndResolveScope(context);
  const filters: AnalyticsFilters = data.filters || {};
  const page = data.page || 1;
  const pageSize = data.pageSize || 10;
  ...
  const unvisitedPhysicians = physicians.filter(p => {
    ...
    return noVisit && filterRecordByScopeAndFilters(p, scope, filters);
  });
  ...
  const paginated = paginateResults(combinedItems, page, pageSize);
 ...
 */
export const getVisitedCustomersAnalytics = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  const { scope } = await enforceAuthAndResolveScope(context);
  const filters: AnalyticsFilters = data.filters || {};
  const page = data.page || 1;
  const pageSize = data.pageSize || 10;
  ...
  const visitedPhysicians = physicians.filter(p => {
    ...
    return hasVisit && filterRecordByScopeAndFilters(p, scope, filters);
  });
  ...
  const totalAssignedCount = physicians.filter(p => filterRecordByScopeAndFilters(p, scope, filters)).length +
                             pharmacies.filter(p => filterRecordByScopeAndFilters(p, scope, filters)).length;
 ...
 */
export const getAnalyticsTrend = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  const { scope } = await enforceAuthAndResolveScope(context);
  const filters: AnalyticsFilters = data.filters || {};
  ...
  // Filter records
  const filteredPhysVisits = physicianVisits.filter(v => filterRecordByScopeAndFilters(v, scope, filters));
  const filteredPharmVisits = pharmacyVisits.filter(v => filterRecordByScopeAndFilters(v, scope, filters));
  const filteredOrders = orders.filter(o => filterRecordByScopeAndFilters(o, scope, filters));

  // Dynamic monthly buckets mapping
  const trendBuckets: Record<string, { medicalVisits: number; pharmacyVisits: number; sales: number }> = {};

  const addVisitToBucket = (dateStr: string | undefined, type: "med" | "pharm", amt: number = 0) => {
    if (!dateStr) return;
    const month = dateStr.substring(0, 7); // YYYY-MM
    if (!trendBuckets[month]) {
      trendBuckets[month] = { medicalVisits: 0, pharmacyVisits: 0, sales: 0 };
    }
    if (type === "med") trendBuckets[month].medicalVisits += 1;
    if (type === "pharm") trendBuckets[month].pharmacyVisits += 1;
 ...
 */
export const getRepPerformanceAnalytics = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  const { scope } = await enforceAuthAndResolveScope(context);
  const filters: AnalyticsFilters = data.filters || {};
  const page = data.page || 1;
  const pageSize = data.pageSize || 10;
  ...
  const repPerformanceList = reps.map(rep => {
    const medVisits = physicianVisits.filter(v => v.repId === rep.id && filterRecordByScopeAndFilters(v, scope, filters));
    const pharmVisits = pharmacyVisits.filter(v => v.repId === rep.id && filterRecordByScopeAndFilters(v, scope, filters));
  ...
  const paginated = paginateResults(repPerformanceList, page, pageSize);
 ...
 */
export const getTerritorySynergyAnalytics = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  const { scope } = await enforceAuthAndResolveScope(context);
  const filters: AnalyticsFilters = data.filters || {};
  ...
  const synergyScores = territories.map(t => {
    ...
    const medVisits = physicianVisits.filter(v => {
      ...
      return match && filterRecordByScopeAndFilters(v, scope, filters);
    });
 ...
 */
export const getVisitQualityAnalytics = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  const { scope } = await enforceAuthAndResolveScope(context);
  const filters: AnalyticsFilters = data.filters || {};
  const page = data.page || 1;
  const pageSize = data.pageSize || 10;
  ...
  const qualityVisits = physicianVisits.filter(v => filterRecordByScopeAndFilters(v, scope, filters)).map(v => {
  ...
  const paginated = paginateResults(qualityVisits, page, pageSize);
 ...
 */
export const getSalesVisitQualityAnalytics = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  const { scope } = await enforceAuthAndResolveScope(context);
  const filters: AnalyticsFilters = data.filters || {};
  const page = data.page || 1;
  const pageSize = data.pageSize || 10;
  ...
  const pharmacyVisits = await getCollectionArray("pharmacyVisits");
  const filteredVisits = pharmacyVisits.filter(v => filterRecordByScopeAndFilters(v, scope, filters));
  ...
  const paginated = paginateResults(qualityItems, page, pageSize);
 ...
 */
export const getSampleAnalytics = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  const { scope } = await enforceAuthAndResolveScope(context);
  const filters: AnalyticsFilters = data.filters || {};
  ...
  const filteredAllocations = allocations.filter(a => filterRecordByScopeAndFilters(a, scope, filters));
  const filteredRequests = requests.filter(r => filterRecordByScopeAndFilters(r, scope, filters));
 ...
 */
export const getInventoryAnalytics = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  const { scope } = await enforceAuthAndResolveScope(context);
 ...
 */
export const getFinanceApprovalAnalytics = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  const { scope } = await enforceAuthAndResolveScope(context);
  const filters: AnalyticsFilters = data.filters || {};
  ...
  const filteredOrders = orders.filter(o => filterRecordByScopeAndFilters(o, scope, filters));
  const filteredPharmacies = pharmacies.filter(p => filterRecordByScopeAndFilters(p, scope, filters));
